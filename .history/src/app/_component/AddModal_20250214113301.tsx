...
interface AddModalProps {
  currentAlcoholDate: Date;
  closeAddModal(): void;
}

export default function AddModal({ currentAlcoholDate, closeAddModal }: AddModalProps) {
  // useState
  ...
  const { addAlcoholDate } = useAddAlcoholDate(() => {
    toast(`${dayjs(currentAlcoholDate).format("YYYY년 MM월 DD일")}에 추가되었습니다.`);
  });
  ...
  function handleSubmit(e: FormEvent) {
    ...
    const id = uuid();
    const date = `${dayjs(currentAlcoholDate).format("YYYY-MM-DD")} 23:00:00`;
  ...
  return (
    <form className="border rounded-lg p-10 flex flex-col gap-8 shadow-md shadow-gray-400" onSubmit={handleSubmit}>
