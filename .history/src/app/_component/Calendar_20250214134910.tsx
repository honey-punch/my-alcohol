...
import { useDeleteAlcoholDate } from "@/hooks/alcohol/alcohol";
...
export default function Calendar({ alcoholDates }: CalendarProps) {
  ...
  // hooks
  const { deleteAlcoholDate } = useDeleteAlcoholDate(() => {
    toast("삭제되었습니다.", { type: "info" });
  });
  ...
  useEffect(() => {
    function handleClickOutsidePopupMenu(event: MouseEvent) {
    ...
    if (popupMenuOpen) {
      document.addEventListener("click", handleClickOutsidePopupMenu);
    }

    return () => {
      document.removeEventListener("click", handleClickOutsidePopupMenu);
    };
  }, [popupMenuOpen]);
  ...
  function handlePopupMenu(value: boolean) {
    setPopupMenuOpen(value);
  }

  function handleClickDeleteButton(day: string) {
    const id = currentMonthDates.find((v) => {
      const newDate = new Date(v.date);
    ...
    if (!id) {
      toast("삭제할 데이터가 없습니다.", { type: "warning" });
      return;
    }
    deleteAlcoholDate(id);
    ...
    <div className="">
          ...
          return (
            <div key={key} className="relative">
              ...
              {popupMenuOpen && isSelectedMonth() && isSelectedDate(day) && (
                <ul
                  ref={popupMenuRef}
                  className="absolute w-[100px] z-10 top-[25px] -right-[110px] bg-white shadow-lg rounded-lg border py-2"
                >
                  <li
                    onClick={() => {
                      handleClickAddButton(true);
                    }}
                    className="py-2 flex items-center justify-center cursor-pointer hover:bg-gray-200"
                  >
                    추가
                  </li>
                  <div className="h-[0.5px] bg-gray-200"></div>
                  <li className="py-2 flex items-center justify-center cursor-pointer hover:bg-gray-200">수정</li>
                  <div className="h-[0.5px] bg-gray-200"></div>
                  <li
                    onClick={() => {
                      handleClickDeleteButton(day);
                    }}
                    className="py-2 flex items-center justify-center cursor-pointer hover:bg-red-500 hover:text-white"
                  >
                    삭제
                  </li>
                </ul>
      ...
      {addModalOpen && (
        <div className="absolute bottom-[116px] right-[116px]" ref={addModalRef}>
          <AddModal currentDate={currentDate} closeAddModal={closeAddModal} handlePopupMenu={handlePopupMenu} />
        </div>
      )}
