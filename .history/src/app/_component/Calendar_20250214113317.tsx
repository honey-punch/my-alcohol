...
export default function Calendar({ alcoholDates }: CalendarProps) {
  ...
  const [currentAlcoholDate, setCurrentAlcoholDate] = useState<AlcoholDate>(today);
  ...
  function handleClickAddButton(value: boolean) {
    const isDuplicateIndex = alcoholDates.findIndex((v) => {
      if (`${dayjs(currentAlcoholDate).format("YYYY-MM-DD")}T23:00:00` === v.date) {
        return v;
      }
    });
  ...
  function isSelectedDate(day: string) {
    const currentAlcoholDateformat = dayjs(currentAlcoholDate).format(DATE_FORMAT);

    let selectedDay = currentAlcoholDateformat.split("-")[2];
  ...
  function isSelectedMonth() {
    ...
    let selectedMonth = currentAlcoholDateformat.split("-")[1];
    ...
    <div className="">
      ...
      {addModalOpen && (
        <div className="absolute bottom-[116px] right-[116px]" ref={addModalRef}>
          <AddModal currentAlcoholDate={currentAlcoholDate} closeAddModal={closeAddModal} />
        </div>
      )}
