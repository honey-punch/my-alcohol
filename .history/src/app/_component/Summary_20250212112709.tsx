...
export default function Summary({ alcoholDates }: SummaryProps) {
  ...
  const sojuAverage = alcoholDates.map((v) => v.soju).reduce((a, b) => a + b);
  const beerAverage = alcoholDates.map((v) => v.beer).reduce((a, b) => a + b);
  ...
  const sojuSavingMoney = sojuPricePerOneBottle * sojuAverage;
  const beerSavingMoney = beerPricePerOneMl * beerAverage;
