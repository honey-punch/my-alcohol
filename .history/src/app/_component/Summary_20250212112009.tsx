...
interface SummaryProps {
  ...
  isLoadingAlcoholDates: boolean;
}

export default function Summary({
  alcoholDates,
  isLoadingAlcoholDates,
}: SummaryProps) {
  const today = new Date();
  const formattedToday = dayjs(today).format(DATE_FORMAT);

  // 1, 2번 박스
  // const [lastDate, setLastDate] = useState<string>(formattedToday);
  // const lastDay = new Date(lastDate);
  const lastDay = new Date(alcoholDates[alcoholDates.length - 1].date);
  ...
  // useEffect(() => {
  //   if (alcoholDates.length > 0) {
  //     setLastDate(alcoholDates[alcoholDates.length - 1].date);
  //   }
  // }, [alcoholDates]);
